...
import warnings
import tensorflow.compat.v1 as tfCon
...
session_conf = tfCon.ConfigProto(intra_op_parallelism_threads=1, inter_op_parallelism_threads=1)
sess = tfCon.Session(graph=tfCon.get_default_graph(), config=session_conf)
K.set_session(sess)
...
# get data
df = read_data()
Y = np.array(df.pop('Class_attribute')).astype(np.float32)
